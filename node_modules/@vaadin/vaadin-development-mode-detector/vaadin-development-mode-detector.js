...
function isUnbundled() {
  if (useHtmlImports) {
    ...
    return htmlImports.filter(function (href) {
      return endsWith(href, "polymer/polymer-element.html");
    }).length > 0;
  } else {
    const scripts = Array.from(document.querySelectorAll("script")); // TODO(web-padawan): this is not expected to work for applications using native ES modules,
...
if (typeof window.Vaadin.developmentMode === "undefined") {
  ...
  const loadAndRun = function (id, optionalArgument) {
    ...
    if (script.parentNode == null) {
      document.body.appendChild(script); // if the script already loaded, dispatch a fake load event
